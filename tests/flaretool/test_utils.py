...
class UtilsTestCase(unittest.TestCase):
    ...
    def test_han_to_zen_kana_protected_base_not_merged(self):
        """non_convert_charsで保護された文字は濁点/半濁点と合成されない（バグ修正の確認）"""
        # 保護された 'ｶ' は 'ガ' に合成されず、後続の 'ﾞ' は単独で '゛' に変換される
        result = convert_value("ｶﾞ", ConversionMode.FULL_WIDTH, non_convert_chars=["ｶ"])
        self.assertEqual(result, "ｶ゛")

        # 半濁点も同様
        result = convert_value("ﾊﾟ", ConversionMode.FULL_WIDTH, non_convert_chars=["ﾊ"])
        self.assertEqual(result, "ﾊ゜")

        # 保護されていない文字は従来通り合成される
        result = convert_value("ｶﾞｷﾞ", ConversionMode.FULL_WIDTH, non_convert_chars=["ｷ"])
        self.assertEqual(result, "ガｷ゛")

    def test_han_to_zen_kana_protected_dakuten_itself(self):
        """'ﾞ'自体を保護した場合は前の文字のみ変換され合成されない"""
        result = convert_value("ｶﾞ", ConversionMode.FULL_WIDTH, non_convert_chars=["ﾞ"])
        self.assertEqual(result, "カﾞ")

        result = convert_value("ﾊﾟ", ConversionMode.FULL_WIDTH, non_convert_chars=["ﾟ"])
        self.assertEqual(result, "ハﾟ")
