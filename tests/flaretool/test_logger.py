...
class TestLogging(unittest.TestCase):
    ...
    def test_setup_logger_rotating_file(self):
        logger = setup_logger(loglevel=logging.DEBUG, file=True, rotating=True)
        self.assertIsInstance(logger, logging.Logger)
        self.assertEqual(len(logger.handlers), 1)
    ...
    def test_setup_logger_multiple_handlers(self):
        logger = setup_logger(loglevel=logging.DEBUG,
                              console=True, file=True, file_name='test.log')
        self.assertIsInstance(logger, logging.Logger)
    ...
    def test_setup_logger_extra(self):
        logger = setup_logger(loglevel=logging.DEBUG, console=True, extra=True)
        self.assertIsInstance(logger, logging.Logger)
        self.assertEqual(len(logger.handlers), 1)
        self.assertIsInstance(logger.handlers[0], logging.StreamHandler)
    ...
    def test_setup_logger_custom_values(self):
        logger = setup_logger(loglevel=logging.DEBUG, console=True,
                              file=True, file_name='test.log', rotating=True)
        self.assertIsInstance(logger, logging.Logger)
