...
class NettoolTest(unittest.TestCase):
    def test_get_global_ipaddr_info(self):
        ...
        with patch("flaretool.common.requests.get") as mock_get:
            ...
            self.assertEqual(
                str(ip_info),
                "IpInfo(ipaddr='192.0.2.1', hostname='example.com', country='JP')",
            )
    ...
    def test_is_country_ip_success(self):
        # 日本のIPアドレスの場合のテスト
        with patch(
            "flaretool.nettool.common.get_country_ip_list"
        ) as mock_get_japanip_list:
            mock_get_japanip_list.return_value = ["192.0.2.0/24", "198.51.100.0/24"]
    ...
    def test_is_japan_ip(self):
        # 日本のIPアドレスの場合のテスト
        with patch(
            "flaretool.nettool.common.get_japanip_list"
        ) as mock_get_japanip_list:
            mock_get_japanip_list.return_value = ["192.0.2.0/24", "198.51.100.0/24"]
    ...
    def test_get_adhost(self):
        # ホストリストを取得する場合のテスト
        with patch("flaretool.common.requests.get") as mock_get:
            mock_get.return_value.text = "example.com\nsub.example.com\n"
    ...
    def test_get_adhost_strips_comments(self):
        # コメント行は除外し、行内コメントはコメント部分のみ除去するテスト
        with patch("flaretool.common.requests.get") as mock_get:
            mock_get.return_value.text = (
                "# full comment line\nexample.com\nsub.example.com # inline comment\n\n"
            )
            host_list = get_adhost()
            self.assertEqual(host_list, ["example.com", "sub.example.com"])
